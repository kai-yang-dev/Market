...
const socketMiddleware: Middleware = (store) => {
  return (next) => (action) => {
    ...
    if (type === SOCKET_CONNECT) {
      ...
      if (!token) {
        console.warn('❌ No admin access token found, cannot connect to socket');
        store.dispatch(socketError('No admin access token available'));
        return next(action);
      }
      ...
      socket.on('disconnect', (reason) => {
        ...
        if (reason === 'io server disconnect') {
          ...
          const tokenNow = localStorage.getItem('adminAccessToken');
